...
# there is strong evidence that it is harmful to human health. In the United States, 
# the Environmental Protection Agency (EPA) is tasked with setting national ambient air 
# quality standards for fine PM and for tracking the emissions of this pollutant into the atmosphere. 
# Approximatly every 3 years, the EPA releases its database on emissions of PM2.5. 
# This database is known as the National Emissions Inventory (NEI). You can read more 
...
# emitted from that source over the course of the entire year. 
...
# PM2.5 emissions data for 1999, 2002, 2005, and 2008. For each year, 
...
# Inventory database and see what it say about fine particulate matter pollution in the 
...
a1 <- select(summary_SCC_df, year, Emissions)
a2 <- group_by(a1, year)
a3 <- summarise(a2, totEmissions = sum(Emissions))

dev.off()

png("Project2_Plot1.png", width = 480, height = 480)

plot(a3, type = "n",
     main = "Total Emissions in US (1999 - 2008)",
     xlab = "Years",
